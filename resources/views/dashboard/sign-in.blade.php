<x-ui-dash.layout :IS_AUTH="true" :PAGE_TITLE="'Sign In'">
    <x-slot name="slot">
        <div class="page-header align-items-start min-vh-100"
            style="background-image: url('{{ asset('app/assets/img/illustrations/illustration-verification.jpg') }}');">
            ...
            @if (session('success'))
                <x-ui-dash.ui.alert :IS_ERROR="false" :HEAD="'Done'" :TITLE="session('success')" />
            @endif

            @if ($errors->any())
                @foreach ($errors->all() as $error)
                    <x-ui-dash.ui.alert :IS_ERROR="true" HEAD="A problem occurred" :TITLE="$error" />
                @endforeach
            @endif
